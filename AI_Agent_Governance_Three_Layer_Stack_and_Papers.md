...
### 30. He et al., 2024 — Does Prompt Formatting Have Any Impact on LLM Performance?

- **arXiv ID:** 2411.10541
- **URL:** https://arxiv.org/abs/2411.10541
- **PDF:** https://arxiv.org/pdf/2411.10541
- **Authors:** Jingying He, Mukund Rungta, David Koleczek, Arshdeep Sekhon, Fangcong Yin Wang, Sajjad Hasan
- **Date:** 2024
- **Citations:** 228

**Abstract:**
> We investigate whether the *syntax* of a prompt changes an AI's behavior when the actual words and instructions remain identical. We isolate the content of a prompt (the persona, task instructions, few-shot examples, and user request) and wrap it in different structural syntaxes: plain text, Markdown, JSON, and YAML. We evaluate these formats across reasoning, code generation, and translation tasks.

**Key findings:**
- **Massive Sensitivity:** Smaller or mid-tier models are highly sensitive to format. GPT-3.5-turbo's accuracy varied by up to **40% in code translation tasks** purely based on how the prompt was structured.
- **Why Markdown Wins Over Plain Text:** Lightweight markup languages like Markdown introduce explicit logical segmentation. The hierarchical syntax (headings and bullet points) acts as structural cues that help the model separate instructions from background data, mitigating ambiguity and improving fidelity to multi-step directives.
- **Larger models (like GPT-4) are more robust** but still exhibit noticeable performance variations depending on layout.

**Why I recommend this paper:**
This is the **foundational controlled experiment** proving that Markdown formatting is not decorative — it is a **behavioral signal**. The 40% performance swing with identical content is a staggering result. It means that the choice of Markdown vs. plain text vs. JSON is not an aesthetic preference; it is a **governance decision** that affects whether the agent follows instructions correctly.

**Relevance to our topic:**
**Layer 0 (Spec-Level Governance)** — This paper directly addresses how skill specifications should be formatted. If a skill is written in Markdown with clear headers (# Role, ## Skills, ### Constraints), the agent will parse it differently than if it were written in plain text. This explains why skill format standards (like GovernSpec's 12 contract sections) matter: the structure itself changes behavior.

**Which layer:** **Layer 0 — Spec-Level Governance**

**Is it a solution we're looking into?** Yes. The formatting of skill markdown is a first-class governance concern. He et al. proves that Markdown should be the mandated format for skill files because it provides explicit logical segmentation that improves instruction fidelity.

**Recommendation reason:** The 40% performance swing is the most compelling empirical evidence that skill format matters. This paper should be the reference for any skill specification standard: Markdown is not optional; it is the format that maximizes agent comprehension and compliance.

---

### 31. Gloaguen et al., 2026 — Evaluating AGENTS.md: Are Repository-Level Context Files Helpful for Coding Agents?

- **arXiv ID:** 2602.11988
- **URL:** https://arxiv.org/abs/2602.11988
- **PDF:** https://arxiv.org/pdf/2602.11988
- **Authors:** Tristan Gloaguen, Niklas Mündler, Markus Müller, Veselin Raychev, Martin Vechev
- **Date:** 2026
- **Citations:** 13

**Abstract:**
> We evaluate the effectiveness of repository-level Markdown files (like `AGENTS.md` or `CLAUDE.md`) across hundreds of real-world tasks. We track the execution paths of agents to see if they genuinely alter their actions based on the Markdown guidelines.

**Key findings:**
- **Proven Compliance:** Agents **absolutely read and respect** instructions written in these files. When a specific repository-level developer tool was documented in the Markdown file, the agent's utilization of that tool jumped from an average of **0.05 calls per task to 2.5 calls** — a **50x increase** in behavioral compliance.
- **The "Double-Edged Sword" Effect:** Agents treat every single instruction in a Markdown file as a strict constraint to satisfy. If the Markdown file is too bloated or automatically generated by an LLM, it over-complicates the task. LLM-generated context files actually **decreased agent success rates by 3%** and inflated reasoning costs by **more than 20%** because the agent wasted time performing unnecessary tests and file lookups.
- **The Optimal Approach:** Clean, minimal, human-written Markdown files focused strictly on essential rules improved task success rates by an average of **4%**.

**Why I recommend this paper:**
This is the **most direct causal evidence** that skill markdown (`AGENTS.md`) is not just read — it is **actively enforced** by agents. The 50x tool-usage spike is irrefutable proof that agents parse and act on markdown instructions. The "Double-Edged Sword" finding is equally important: it proves that **skill quality matters**, not just skill existence. Bloated, LLM-generated skills hurt performance. This is a governance issue: who writes the skills, and how do we prevent skill bloat?

**Relevance to our topic:**
**Layer 0 (Spec-Level Governance)** — This paper is the empirical validation of what we've been claiming: skill markdown files are operational constraints, not documentation. The 50x compliance spike proves the causal link. The 3% decrease from bloated files proves that governance must include **skill authoring standards** (brevity, focus, human curation) and **skill validation** (detecting bloat before deployment).

**Which layer:** **Layer 0 — Spec-Level Governance**

**Is it a solution we're looking into?** Yes. This paper provides the empirical foundation for:
1. Mandating `AGENTS.md` or equivalent skill files for all repositories
2. Enforcing skill brevity (preventing LLM-generated bloat)
3. Measuring skill effectiveness by tracking agent execution paths (behavioral trace analysis)

**Recommendation reason:** The 50x compliance spike is the single most compelling evidence that skill markdown affects behavior. The "Double-Edged Sword" finding transforms skill governance from "do we have a skill file?" to "is the skill file well-written?" This is a critical distinction that no other paper makes.

---

### 32. Chen et al., 2025 — MDEval: Evaluating and Enhancing Markdown Awareness in Large Language Models

- **arXiv ID:** 2501.15000
- **URL:** https://arxiv.org/abs/2501.15000
- **PDF:** https://arxiv.org/pdf/2501.15000
- **Authors:** Zhenyu Chen, Yutong Liu, Lei Shi, Xiang Chen, Yiran Zhao, Fengjun Ren
- **Date:** 2025
- **Citations:** 14

**Abstract:**
> We develop MDEval, a benchmark with 20,000 instances designed to score an LLM's "Markdown Awareness" — its ability to natively comprehend Markdown layout. We evaluate whether Markdown awareness correlates with structural instruction-following capability and whether fine-tuning for Markdown structure improves performance.

**Key findings:**
- **Markdown Awareness Correlates with Instruction Following:** A model's ability to natively comprehend Markdown layout directly correlates with its overall structural instruction-following capability.
- **Fine-Tuning Closes the Gap:** Open-source models fine-tuned specifically to recognize and generate Markdown structures can close the performance gap with frontier models like GPT-4o regarding task layout compliance.
- **Human Correlation:** High MDEval scores closely match human standards for reading and logical flow.

**Why I recommend this paper:**
MDEval provides the **measurable metric** for why Markdown works as a skill format. It's not just convention — it's because LLMs have **learned structural parsing** from their training data, and Markdown's explicit syntax (headers, lists, code blocks) aligns with how models internally segment context. The benchmark gives us a tool to evaluate whether a model is capable of following structured skill instructions before we deploy it.

**Relevance to our topic:**
**Layer 0 (Spec-Level Governance)** — MDEval is a **model-selection governance tool**. If a model scores low on Markdown Awareness, it will be unreliable at following skill instructions regardless of how well the skill is written. This means governance is not just about the skill file — it's about the **skill + model pair**. A well-written skill with a low-MDEval model is still a governance failure.

**Which layer:** **Layer 0 — Spec-Level Governance**

**Is it a solution we're looking into?** Yes. MDEval should be part of the skill deployment pipeline:
1. **Model screening:** Only deploy agents with models scoring above a threshold on MDEval
2. **Skill format validation:** Ensure skills use Markdown structures that MDEval measures (headers, lists, code blocks)
3. **Fine-tuning guidance:** If using open-source models, fine-tune for Markdown Awareness to improve skill compliance

**Recommendation reason:** MDEval explains the mechanism behind He et al.'s 40% swing and Gloaguen et al.'s 50x compliance spike. It proves that Markdown structure is not just a human preference — it is a **model-comprehensible semantic layer**. The benchmark turns "use Markdown" from a best-practice guess into a **measurable, enforceable standard**.
...
- Written for humans who can apply judgment (e.g., "use your discretion for refunds")
- Missing edge cases that only emerge during execution (e.g., "what if the customer is both a VIP AND a fraud risk?")
- Containing tacit knowledge that the author never verbalized (e.g., "I always check the weather before shipping fragile items")

### The Four-Step Process That No Formal Paper Addresses

The following methodology is used in enterprise agent deployments to convert human SOPs into executable workflows. It reveals why the gap between "human intent" and "formal spec" is the hardest part of agent governance.

#### Step 1: Format Standardization

**What it does:** Convert unstructured SOP prose into machine-readable, parameterized instructions.

**Key techniques:**
- **Parameterization:** Replace fixed values with variables (e.g., "wash on normal" → `mode: {quick|normal|delicate}, temperature: {cold|warm|hot}`)
- **MUST/SHOULD/MAY rules:** Use RFC 2119 language to categorize rules by importance
  - **MUST:** Mandatory steps (e.g., "MUST check for empty pockets")
  - **SHOULD:** Recommended but can be skipped with justification (e.g., "SHOULD pre-treat stains")
  - **MAY:** Optional steps (e.g., "MAY add fabric softener")
- **Structured Markdown:** Divide into Parameters, Steps, Error Handling for machine parseability

**Why formal methods fail:** ZipperGen assumes parameters and types are already defined. It does not help decide what should be parameterized, how to extract implicit knowledge, or how to categorize rules by importance.

**Example — Laundry SOP conversion:**

```markdown
## Human SOP (original)
"Wash the clothes normally. Don't forget to check for stains."

## Standardized Skill (converted)
### Parameters
- load_size: {small|medium|large}
- fabric_type: {delicate|normal|heavy}
- temperature: {cold|warm|hot} — MUST default to cold for delicate

### Steps
1. MUST classify fabric type before selecting cycle
2. SHOULD pre-treat visible stains (skip if time < 15 min)
3. MUST set temperature based on fabric_type mapping
4. MAY add fabric softener if fabric_type == normal

### Error Handling
- IF fabric_type == delicate AND temperature == hot → MUST abort with error
```

The conversion from three sentences to a parameterized skill requires **domain expertise, judgment, and knowledge of what the agent cannot infer** — none of which ZipperGen provides.

#### Step 2: Task Decomposition

**What it does:** Split a large task into smaller, independent pipeline steps with explicit inputs, outputs, and success criteria.

**Key techniques:**
- Define each step as a separate skill or small agent
- Connect steps via structured artifacts (e.g., JSON files) — no implicit communication
- Explicitly define success criteria per step (e.g., "classification accuracy > 95%")

**Why formal methods fail:** ZipperGen assumes the global workflow is already decomposed into lifelines. It does not help decide:
- What should be a "skill" vs. an "agent" vs. a "workflow step"
- How to scope a skill (too broad → unreliable; too narrow → inefficiency)
- How to define the message payloads (the video shows structured JSON artifacts, not just abstract message types)

**Example — Customer ticket handling:**

```
Original SOP: "Handle the customer ticket."

Decomposed workflow:
1. [Agent: Triage] Input: ticket text, employee_id → Output: JSON {category, priority, assignee}
2. [Agent: Research] Input: category → Output: JSON {kb_articles, similar_cases}
3. [Agent: Draft] Input: ticket + research → Output: JSON {response_text, confidence}
4. [Agent: QC] Input: draft + ticket → Output: JSON {approved, corrections}
5. [Human: Escalation] IF priority == urgent OR confidence < 0.8 → Human review
```

Each step is a separate skill. The JSON artifacts are the contracts between steps. ZipperGen could verify that these messages are sent and received in the correct order (deadlock-free). But it cannot tell you **what should be in the JSON**, **which fields are mandatory**, or **what the scope of each agent should be**.

#### Step 3: Bidirectional Development (Iterative Refinement)

**What it does:** Deploy the workflow, observe failures, refine the SOP, and redeploy. Repeat until robust.

**Key insight:** *"Initial SOPs inevitably omit tacit knowledge. Real-world errors expose gaps and ambiguities."*

**Example iterations from the video:**

| Iteration | Failure | SOP Update | What ZipperGen Misses |
|-----------|---------|------------|----------------------|
| 1 | Clothes shrink after drying | Add: "IF fabric_type == delicate AND temperature == hot → MUST abort" | ZipperGen assumes specs are static. No mechanism for learning from failures. |
| 2 | Stain remains after washing | Add: "MUST pre-treat oil stains separately" | ZipperGen assumes all conditions are pre-specified. No handling of unanticipated failure modes. |
| 3 | High-value item damaged | Add: "IF value > $5000 → MUST pause for human approval" | ZipperGen has no concept of "human decider" with latency, uncertainty, or refusal. |

**Why formal methods fail:** Formal specifications are static. They do not incorporate runtime feedback into the specification itself. The video's methodology treats the SOP as a **living document** that evolves through deployment cycles. ZipperGen takes a spec as input and produces a program — it does not help you **discover what the spec should be**.

#### Step 4: Integration & Execution

**What it does:** Connect the workflow to real-world tools (Notion, Jira, APIs) via MCP; add human-in-the-loop checkpoints.

**Key techniques:**
- **MCP integration:** Standardize tool access across platforms (ChatGPT, Claude, Cursor)
- **Human-in-the-loop:** Pause execution at critical points (e.g., finance > $5000) for human approval
- **Error handling:** Real-world API failures, retries, timeouts, rate limits

**Why formal methods fail:** ZipperGen assumes perfect FIFO channels between agents. It does not model:
- API timeouts and retries
- Rate limits that cause temporary unavailability
- Human decision latency (minutes to hours)
- Tool version changes that break message schemas
- Partial failures where one agent succeeds but another fails

### The Verdict: ZipperGen Is a Coordination Compiler, Not a Workflow Designer

> **ZipperGen guarantees that a formal specification will execute correctly.** But the hardest part of agent governance is not executing a spec — it's **creating the spec from human intent**. The SOP-to-workflow conversion process (parameterization, decomposition, iterative refinement, integration) is where most agent projects fail. ZipperGen, ACP, ABC, and all other formal-methods papers assume this hard work is already done. Future research must bridge the gap between human SOPs and formal specifications, not just between formal specifications and correct execution.

### Implications for the Three-Layer Stack

| Layer | Current State | Missing Piece |
|-------|--------------|---------------|
| **Layer 0 (Spec)** | GovernSpec, SSL, ZipperGen | **SOP-to-spec extraction** — tools that convert human SOPs (with tacit knowledge) into formal specs, iteratively refined |
| **Layer 1 (Runtime)** | ACP, ABC, ZipperGen | **Human-in-the-loop integration** — formal verification that accounts for human decision latency and uncertainty |
| **Layer 2 (Behavioral)** | BeSafe-Bench, ST-WebAgentBench | **Tacit knowledge evaluation** — benchmarks that measure whether agents can infer unstated assumptions |

The gap is not just technical — it is **epistemological**. How do we convert what humans know but cannot say into what machines can execute without error? No paper in the current literature addresses this.
